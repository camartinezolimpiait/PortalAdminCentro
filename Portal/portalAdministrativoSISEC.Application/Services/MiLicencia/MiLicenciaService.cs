...
using Categoria = portalAdministrativoSISEC.Application.Data.CompraPin.Categoria;
...
namespace portalAdministrativoSISEC.Services.MiLicencia
{
    public class MiLicenciaService : IMiLicenciaService
    {
        ...
        public async Task<List<Categoria>> ObtenerCategorias()
        {
            ...
            try
            {
                categorias = await _apiMilicenciaService.ConsumirServicioGET<List<Categoria>>(MetodosApiFrontMiLicencia.GET_CATEGORIAS, true);
                return categorias;
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <returns></returns>
        public async Task<List<Categoria>> ObtenerCategoriasCrc()
        {
            List<Categoria> categorias = new();
            try
            {
                categorias = await _apiMilicenciaService.ConsumirServicioGET<List<Categoria>>(MetodosApiFrontMiLicencia.OBTENER_CATEGORIAS, true);
                return categorias;
            }
            catch (Exception ex)
            {
        ...
        /// <param name="idCentro"></param>
        /// <returns></returns>
        public async Task<List<Categoria>> ConsultaCategoriasCentro(long idCentro)
        {
            try
            {
                return await _apiMilicenciaService.ConsumirServicioGET<List<Categoria>>($"{MetodosApiFrontMiLicencia.OBTENER_CATEGORIAS_CENTROS}/{idCentro}", true);
            }
            catch (Exception ex)
            {
                string methodName = System.Reflection.MethodBase.GetCurrentMethod().Name ?? "Default";
        ...
        public async Task<ResponseDTO<ValidateCouponCompanyResponse>> ValidateCoupon(ValidateCouponRequest request)
        {
            try
            {
                ResponseDTO<ValidateCouponCompanyResponse> response = await _apiMilicenciaService
                    .ConsumirServicioPOST<ResponseDTO<ValidateCouponCompanyResponse>>(
                        MetodosApiFrontMiLicencia.VALIDATE_COUPON,
                        request,
                        true
                    );

                return response ?? new ResponseDTO<ValidateCouponCompanyResponse>
                {
                    Codigo = -1,
            ...
            {
                await SaveLog(request, ex, new StackFrame(1).GetMethod()?.Name);
                return new ResponseDTO<ValidateCouponCompanyResponse>
                {
                    Codigo = -1,
                    Respuesta = "Error al validar el cupón",
                    Entidad = null
                };
            }
        }
        ...
        public async Task<ResponseDTO<List<string>>> ObtenerEmpresasPines(string codigoRunt)
        {
            try
            {
                return await _portalAdministrativoService
                    .ConsumirServicioGET<ResponseDTO<List<string>>>(
                        $"{MetodosApiPortalAdmin.OBTENER_EMPRESAS_PINES}?idRunt={codigoRunt}", true);
            }
            catch (Exception ex)
            {
                string methodName = System.Reflection.MethodBase.GetCurrentMethod().Name ?? "Default";
                string componentName = System.Reflection.MethodBase.GetCurrentMethod().DeclaringType.Name ?? "Default";
                await _portalAdministrativoService.RegisterExceptionLog(ExtensionLog.GenerateExceptionLog(ex, methodName, componentName, null, null, null));
                return new ResponseDTO<List<string>> { Codigo = -1, Entidad = new List<string>() };
            }
        }

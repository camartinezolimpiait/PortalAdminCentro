...
namespace portalAdministrativoSISEC.Services.SuperTransporte
{
    public class SuperTransporteService : ISuperTransporteService
    {
        ...
        public async Task<string> GetBlobSasToken()
        {
            ...
            // Crear una referencia al almacenamiento de Azure
            CloudStorageAccount storageAccount = CloudStorageAccount.Parse(connectionString);

            // Crear el cliente de blobs
            CloudBlobClient blobClient = storageAccount.CreateCloudBlobClient();

            // Obtener una referencia al contenedor
            CloudBlobContainer container = blobClient.GetContainerReference(containerName);

            // Crear una firma de acceso compartido (SAS) para el contenedor
            SharedAccessBlobPolicy sasPolicy = new SharedAccessBlobPolicy()
            {
                SharedAccessStartTime = DateTime.UtcNow,
                SharedAccessExpiryTime = DateTime.UtcNow.AddSeconds(30), // Expire en 50 segundos
                Permissions = SharedAccessBlobPermissions.Read // Permiso de solo lectura
            };

            // Generar el token SAS para el contenedor
